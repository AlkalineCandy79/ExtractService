...
# Created:  8/25/2020
# Modified: 03/19/2022
# Modification Purpose:  Added record check to verify good to go before moving
#                        to production.
#
#
#-------------------------------------------------------------------------------
...
path_Proj = r'\\Projects\KCDataPull\KCDataPull.aprx'  #Contains your layer you want to download.

# Data Store Location
path_DataProcessing = r'\\Projects\KCDataPull\KC Data Pull-Processing.sde'
path_DataFinal = r'\\Projects\KCDataPull\KC Data Pull-Final.sde'

# Target Projection
targetSRID = 6597

# Send confirmation of rebuild to
email_target = 'gisdba@gisiscool.com'
email_customer = 'gisdba@gisiscool.com'
email_subjectSchema = 'King County'

# Configure the e-mail server and other info here.
mail_server = 'smtprelay.google.com'
mail_from = 'COB GIS Publishing Message<noreply@gisiscool.com>'

# Portal Configs
portalURL = 'https://www.arcgis.com'
portalUser = 'your arcgis account'
portalPW = 'your arcgis password'  #Base64 Encoded. Remove decode if you don't want to.
...
#-------------------------------------------------------------------------------
    ...
    downloadCheck = int(arcpy.GetCount_management(inFC).getOutput(0))

    # Record check added 2021-03-19 JS
    if downloadCheck > 600000:
        print ('Download check complete. {} records detected.'.format(downloadCheck))

        mail_priority = '5'
        mail_subject = 'Portal Feature Staging QC Success'
        mail_msg = ('Records QC check succeeded. Downloaded record count as follows: \n\n' +
        '{} received. Expected 600K+ (~630,064). Process proceeding to staging.'.format(downloadCheck))

        sentMailNote(mail_priority, mail_subject, mail_msg)

    else:
        print ('Status:  Failure!')
        print ('Record check failed. {} detected. Expected > 600K'.format(downloadCheck))

        mail_priority = '1'
        mail_subject = 'Portal Feature Copier Failure:  Download check failure.'
        mail_msg = ('A record count QC check failed. Downloaded record count as follows: \n\n' +
        '{} received. Expected 600K+ (~630,064)'.format(downloadCheck))
